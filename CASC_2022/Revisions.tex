...
%
\begin{abstract}
Motivated by the extensive work in the companion paper \cite{GPS22a}  submitted to CASC,
we devised, analyzed and tested numerically an algorithm
for computing the $k$th order derivative of the ratio $\frac{p'(x)}{p(x)}$ for a black box univariate polynomial $p(x)$ -- given by a black box for its evaluation.
This is an alternative to the algorithm in \cite{GPS22a}, which also works for a black box univariate polynomial $p(x)$.
...
Our subject is a  novel efficient algorithm for the computation of ratio $\frac{p'(x)}{p(x)}$ for a black box univariate polynomial $p(x)$ -- given by a black box for its evaluation. This goal was
motivated by the discovery in our companion submission to  CASC \cite{GPS22a} of the reduction to this problem of the classic root-squaring DLG iterations by   Dandelin 1826, Loba\-chevsky 1834,  Gr{\"a}ffe 1837(cf. \cite{10.2307/2310626}).
For the sake of completeness
of our exposition we reproduce the relevant material from \cite{GPS22a}
in the Appendix.
...
The authors of \cite{Grenet2015DeterministicRF} apply the DLG formulae to solving polynomials over finite fields. The authors of \cite{Becker2018ANS} apply DLG iterations to the benchmark problem (\emph{i.e.}, the isolation of the roots of a polynomials, See Sec. \ref{srrapp}) which they claim is comparable to the record bound in \cite{pan2002univariate} which at the time was unbroken for 14 years.


This short survey illustrates that the application of the DLG to root-finding, while more than a century old, is still an interesting research topic. One of the novelties and improvements of our algorithms is to
% consider the identities given by Eq.~\ref{eqdndrt}, instead of the usual Eq.~\ref{eqdnd}, to approximate the root radius.
devise, analyze and numerically test an algorithm
for computing the $k$th order derivative of the ratio $\frac{p'(x)}{p(x)}$ for a black box univariate polynomial $p(x)$ -- given by a black box for its evaluation.
This is an alternative to the algorithm in \cite{GPS22a}, which also works for a black box univariate polynomial $p(x)$.
Furthermore, we show that, contrary to intuition, our algorithm is numerically stable and well-behaved when taking the limit of Eq.~\ref{eqdndrt} to 0.
