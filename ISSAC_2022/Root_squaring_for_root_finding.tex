...
In this section we give the results of our experiments where we use DLG iterations to estimate the extremal root radii of the univariate test polynomials included as part of the MPSolve package. To measure accuracy, we compute the relative error of the estimates in comparison to the extremal roots obtained by MPSolve using the formula
$$
\text{relative error}_{r_i} = \frac{|\tilde{r_i} - r_i|}{|r_i|},
$$
where $r_i$ is the root given by MPSolve and $\tilde{r_i}$ our estimate.
...
\begin{table*}[t]
\caption{Experimental Data for Mand}
\label{tab:mand}
\vskip -0.15in
\begin{center}
\begin{small}
