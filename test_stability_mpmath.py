from DLG_alg_mpmath import DLG_rational_form, DLG, add, sub, mul, div, precision
import mpmath as mp 
import numpy as np
	...
	for l in range(1,13):   
		...
		for d in range(2,16):
			...
			for n in range(2,N):
				...
				pert_r = mp.mpf(np.random.uniform(low=-max_delta, high=max_delta))
				pert_i = mp.mpf(np.random.uniform(low=-max_delta, high=max_delta))
				x_pert = add(x, add(pert_r, mul(pert_i,j)))
				delta_x = mp.fabs(add(pert_r,  pert_i*j))
				if delta_x == 0: print("ERROR: zero perturbation")
	
				f = DLG(p, dp, mp.mpc(x), l)
				f_pert = DLG(p, dp, mp.mpc(x_pert), l)
				delta_f = mp.fabs(sub(f_pert,  f))
				abs_rat = div(delta_f, delta_x)
				max_abs_rat = max(max_abs_rat, abs_rat)
				if f == 0:
					#print("ERROR: zero outcome")
					continue
				rel_rat = div(mul(delta_f,mp.fabs(x)),mul(mp.fabs(f),delta_x)),
