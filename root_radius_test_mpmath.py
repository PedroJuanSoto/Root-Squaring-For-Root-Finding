...
p_rev  = lambda x: sub(1,pod(mul(x,rt),d))
dp_rev = lambda x: mul(-d,pod(mul(x,rt),d-1))
...
for l in range(3, l_max+1):
    ...
    for e in range(1,21):
        ...
        print("root=", rt)
        print("error_root=", sub(mp.root(mp.fabs(approx), mp.power(2,l)), rt))
# for l in range(l_min, l_max+3):
#    precision *= 2
#    mp.mp.dps = precision
#    print("l=",l)
#    for e in range(1,20):
...
#                approx = div(DLG(p_rev,dp_rev,sub(0,mul(x,mp.power(2,-e))),l),d)
#                print("approx=",mp.fabs(approx))
#                real = mp.power(div(1,rt),mp.power(2,l))
#                print("radius=", mp.fabs(real))
#                print("error=", float(mp.fabs((mp.fabs(real))-(mp.fabs(approx)))/(mp.fabs(real))))
#                print("approx_root=",float(mp.root(mp.fabs(approx), mp.power(2,l))))
#                print("error_root=", float(sub(mp.root(mp.fabs(approx), mp.power(2,l)), div(1,rt))))
